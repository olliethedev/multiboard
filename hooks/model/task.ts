...
import type { Priority } from '@zenstackhq/runtime/models';

export function useCheckTask<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; title?: string; description?: string; priority?: Priority; order?: number; columnId?: string; assigneeId?: string; isArchived?: boolean }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
